...
class LineAnalyzer
  ...
  #Im lement the initialize() method to:
  ...
  def self.calculate_word_frequency(content)
    word_frequency = content.each_with_object(Hash.new(0)) {|word,count| count[word] +=1}
    puts word_frequency
    puts highest_wf_count = word_frequency.values.max
    #puts hash(value(highest_wf_count))
    
    

    puts word_frequency= word_frequency.reject {|k,v| v != highest_wf_count}  # to filter just the words with higest count

    highest_wf_words = word_frequency.keys  # to remove the values and just have the keys
